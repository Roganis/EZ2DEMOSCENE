...
enum Cmd {
    ...
    Mesh {
        ...
        sdf: bool,
        /// Instances made by the swarm compute pass (in `SwarmGpu::out`).
        gpu: bool,
    },
    Particles {
        slot: u32,
...
/// Big swarms generated by a compute shader (`swarm.wgsl`) straight into
/// a vertex buffer; absent where there are no compute shaders (WebGL2).
struct SwarmGpu {
...
/// One dispatch: a swarm layer seen through one symmetry copy.
struct SwarmJob {
    params: SwarmParams,
    count: u32,
}
...
impl SwarmGpu {
    fn new(device: &wgpu::Device) -> Option<SwarmGpu> {
        ...
        let module = shader(device, "swarm", include_str!("shaders/swarm.wgsl"), false);
    ...
    /// Room for `jobs` dispatches writing `instances` copies in total.
    fn reserve(&mut self, device: &wgpu::Device, jobs: u64, instances: u64) {
        ...
        if changed {
            self.bg = Self::make_bg(device, &self.bgl, &self.params, &self.out);
        }
    }
}
...
impl Renderer {
    ...
    fn render_scene(&mut self, project: &Project, ctx: &EvalCtx, target: &RenderTarget) {
        ...
        for (li, layer) in layers.iter().enumerate().filter(|(_, l)| l.enabled) {
            ...
            match &layer.kind {
                ...
                LayerKind::Mesh(m) => {
                    ...
                    let gpu =
                        self.swarm.is_some() && matches!(m.instancer, Instancer::Swarm { .. });
                    ...
                    {
                        // Placed by the compute pass, one dispatch per
                        // symmetry copy, in the swarm buffer.
                        let n = (*count).min(SWARM_MAX);
                        ...
                        for sym in symmetry_matrices(&layer.symmetry) {
                            swarm_jobs.push(SwarmJob {
                                params: SwarmParams {
                                    ...
                                    shape: [*radius, *spread, *speed as f32, ctx.phase],
                                    ints: [form.index(), n, *seed, swarm_total],
                                    var_a: [
        ...
        if let (Some(sw), false) = (self.swarm.as_mut(), swarm_jobs.is_empty()) {
            sw.reserve(&self.device, swarm_jobs.len() as u64, swarm_total as u64);
    ...
    fn cmd_bounds(&self, cmd: &Cmd, blocks: &[Block]) -> Option<(Vec3, f32)> {
        ...
        match cmd {
            // GPU swarms aren't read back: always drawn.
            Cmd::Mesh { gpu: true, .. } => None,
            Cmd::Mesh {
                mesh,
                first,
                count,
