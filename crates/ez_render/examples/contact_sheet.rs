...
fn main() -> anyhow::Result<()> {
    ...
    let out = args.get(1).cloned().unwrap_or_else(|| "contact_sheet.png".into());
    ...
    let list: Vec<_> = presets::all()
        .into_iter()
        .filter(|p| only.as_ref().is_none_or(|o| p.name.to_lowercase().contains(&o.to_lowercase())))
        .collect();
    ...
    for (row, p) in list.iter().enumerate() {
        for (col, ph) in phases.iter().enumerate() {
            let img = r.render_image(&p.project, &EvalCtx::new(&p.project.timing, *ph, None), &target);
